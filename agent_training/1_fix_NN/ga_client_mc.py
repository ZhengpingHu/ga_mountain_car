...
def calculate_max_energy(pos_history: List[float], vel_history: List[float]) -> float:
    """
    计算轨迹中的最大机械能 (Total Mechanical Energy).
    E = PE + KE
    
    1. Potential Energy (PE):
       MountainCar height formula: h = sin(3 * pos) * 0.45 + 0.55
       PE ~ m*g*h. Here roughly proportional to height.
       Range: 0.0 (valley) to 1.0 (top).
       
    2. Kinetic Energy (KE):
       KE = 0.5 * m * v^2.
       Vel range [-0.07, 0.07]. v^2 max ~ 0.005.
       
    为了让 Agent 重视速度（学会荡起来），我们需要大幅加权 KE。
    Weighting KE by 200 makes it comparable to PE.
    """
    max_e = -999.0
...
class NNPolicy(nn.Module):
    def __init__(self):
        super().__init__()
        # Natural Input: 2 (Pos, Vel) -> Output: 3 (Left, Stop, Right)
        self.net = nn.Linear(2, 3, bias=True)
